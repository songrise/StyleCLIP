...
class Coach:
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.opts.work_in_stylespace:
                    ...
                    w_hat = [
                        c + 0.1 * delta_c for (c, delta_c) in zip(w, delta)]
                    x_hat, _, w_hat = self.net.decoder(
                        [w_hat], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1, input_is_stylespace=True)
                else:
    ...
    def calc_loss(self, w, x, w_hat, x_hat):
        #!Re x and x_hat: generated images
        loss_dict = {}
        loss = 0.0
        if self.opts.id_lambda > 0:
